...
for task in "$2"
	...
	for measure_method in sort_gini
	do
		for measure_variant in "$1"
		do
			...
			for transform_opt in '--log-measure --smooth' #'' # '--sqrt-measure --zero-clip' IMPORTANT NO LOG!!!
			do
				cache_opt=''
				if [[ -n "$transform_opt in '' # '--log-measure --smooth' ''
				then
