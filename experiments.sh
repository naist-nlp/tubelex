...
do
	...
	echo "$corpus"
	python experiments/run.py $corpus_opt     id zh                   --corr --fam id zh > experiments/fam-corr-${corpus}.tsv
	if [[ "$corpus" =~ ^tubelex ]]
	then
		python experiments/run.py $corpus_opt id --form lemma 		  --corr --fam id    > experiments/fam-corr-${corpus}-lemma.tsv
