...
echo
echo gini
python experiments/run.py --cache --cached gini --minus --gini ja --corr --fam ja > experiments/fam-c-gini-corr-gini.tsv
echo csj
python experiments/run.py --cached gini --form lemma --csj   --corr --fam ja > experiments/fam-c-gini-corr-csj-lemma.tsv
echo laborotv
python experiments/run.py --cached gini -D ipadic --laborotv --corr --fam ja > experiments/fam-c-gini-corr-laborotv.tsv
cache_opt='--cached gini'
...
do
	...
	echo "$corpus"
	python experiments/run.py $cache_opt $corpus_opt ja            --corr --fam ja > experiments/fam-c-gini-corr-${corpus}.tsv
	if [[ "$corpus" =~ ^tubelex ]]
	then
		python experiments/run.py $cache_opt $corpus_opt ja --form lemma --corr --fam ja  > experiments/fam-c-gini-corr-${corpus}-lemma.tsv
		python experiments/run.py $cache_opt $corpus_opt ja --form base --corr --fam ja > experiments/fam-c-gini-corr-${corpus}-base.tsv
	fi
done
